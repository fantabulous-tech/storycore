...
namespace IngameDebugConsole
{
	...
	public static class DebugLogConsole
	{
				...
				foreach( var type in assembly.GetExportedTypes() )
				{
					foreach( var method in type.GetMethods( BindingFlags.Static | BindingFlags.Public | BindingFlags.DeclaredOnly ) )
					{
						foreach( var attribute in method.GetCustomAttributes( typeof( ConsoleMethodAttribute ), false ) ) {
							...
							if (consoleMethod != null) { AddCommand(consoleMethod.Command, consoleMethod.Description, type, new [] {method}); }
						}
					}
				}
			}
		...
		public static void LogAllCommands()
		{
			...
			foreach( var entry in methods ) {
				if (entry.Value.IsValid()) {
					Debug.Log("- " + entry.Value.signature);
				}
			}
		}
		...
		private static void AddCommand( string command, string description, string methodName, Type ownerType, object instance = null )
		{
			...
			MethodInfo[] commandMethods = ownerType.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static | BindingFlags.FlattenHierarchy).Where(m => m.Name == methodName).ToArray();
			...
			AddCommand( command, description, ownerType, commandMethods, instance );
		}

		private static void AddCommand( string command, string description, Type ownerType, MethodInfo[] commandMethods, object instance = null ) {

			MethodInfo method = null;
			Type[] parameterTypes = null;

			foreach (MethodInfo possibleMethod in commandMethods) {
				// Fetch the parameters of the class
				ParameterInfo[] parameters = possibleMethod.GetParameters();
			...
			if (method == null) {
				Debug.LogError( command + " does not exist in " + ownerType );
				return;
			}
