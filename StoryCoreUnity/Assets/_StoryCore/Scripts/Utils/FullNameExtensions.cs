...
namespace StoryCore.Utils {
    ...
    public static class FullNameExtensions {
        public static string FullName(this Object o, FullName.Parts parts = Utils.FullName.Parts.Default, int maxLength = 0) {
            if (o == null) {
                return "null";
            }
...
#if UNITY_EDITOR
            if (FlagSet(parts, Utils.FullName.Parts.AssetPath)) {
            ...
            if (FlagSet(parts, Utils.FullName.Parts.Name)) {
                if (contentBeforeName) {
                    builder.Append(" ");
                }
                if (FlagSet(parts, Utils.FullName.Parts.FullScenePath)) {
                    if (t != null) {
            ...
            if (FlagSet(parts, Utils.FullName.Parts.SiblingIndex) && t) {
                builder.AppendFormat("[{0}]", t.GetSiblingIndex());
            }

            if (FlagSet(parts, Utils.FullName.Parts.TypeName)) {
                if (contentBeforeType) {
                    builder.Append(":");
                }

                Type type = o.GetType();
                if (FlagSet(parts, Utils.FullName.Parts.FullTypeName)) {
        ...
        private static void BuildScenePath(Transform t, StringBuilder builder, FullName.Parts parts) {
            if (t.parent != null) {
                ...
                if (FlagSet(parts, Utils.FullName.Parts.SiblingIndex)) {
