							...
							else if (blinkingControlMode == ControlMode.PoseBased)
							{
								...
								for (int b = 0; b < blinkingShape.bones.Count; b++)
								{
									...
									else
									{
										...
										if (!blinkingShape.bones[b].lockRotation)
											blinkingShape.bones[b].bone.localRotation = Quaternion.Lerp(boneShapes[blinkingShape.bones[b].bone].storedRotation, Quaternion.Euler(blinkingShape.bones[b].endRotation), blinkTimer / halfBlinkSpeed);
									}
								}
							}
						}
							...
							else if (blinkingControlMode == ControlMode.PoseBased)
							{
								...
								for (int b = 0; b < blinkingShape.bones.Count; b++)
								{
									...
									else
									{
										...
										if (!blinkingShape.bones[b].lockRotation)
											blinkingShape.bones[b].bone.localRotation = Quaternion.Lerp(Quaternion.Euler(blinkingShape.bones[b].endRotation), boneShapes[blinkingShape.bones[b].bone].storedRotation, (blinkTimer - halfBlinkSpeed) / halfBlinkSpeed);
									}
								}
							}
